...
#--------------------------------------------------------------------------
# In this version, access are done via filefs.
# 
# benchmark : 
#   0.4 ms seconds for writing a value
#   3   ms if export/unexport on each access
#
# TODO : access par direct read/write in MIPS register
#--------------------------------------------------------------------------
...
########################################################################
#  Constants, for Oniion/Omega architectures
########################################################################
...
=end
  ...
  def rgb_board(r,g,b)
    sync_spawn( "fast-gpio","set","17","1") if r<=0
    sync_spawn( "fast-gpio","set","16","1") if g<=0
    sync_spawn( "fast-gpio","set","15","1") if b<=0
    async_spawn( "fast-gpio","pwm","17","200",(r*100/255).to_s) if r>0
    async_spawn( "fast-gpio","pwm","16","200",(g*100/255).to_s) if g>0
    async_spawn( "fast-gpio","pwm","15","200",(b*100/255).to_s) if b>0
  end
end
...
#    Tested with a 128x64 Oled Groove (seedStudio)
...
class OnionOled < ConstOnionGpio
  ...
  def initialize(i2c,noi2c, verbose=false)
    ...
    @opt=["-q"]
    @opt=["-v"] if verbose
    sync_spawn("oled-exp","-ci")
    puts('OLED(%d,%d) ' % [@i2c,@noi2c]) if @verbose 
  end
  def reset()          sync_spawn("oled-exp","-ci","power","on");sleep(0.05);invert(false);sleep(0.05);clear() end
  def clear()          sync_spawn("oled-exp","-c") end
